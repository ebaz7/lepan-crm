...
async function run() {
  ...
  try {
    const q1 = `SELECT 
      ...
      ORDER BY t9.Field_013 ASC, t8.Field_008 ASC
      `;
    const res1 = await axios.post(url, { query: q1 }, { headers });
    res1.data.data.forEach(r => {
        console.log(`DocType: ${r.DocType}, DocNum: ${r.DocNum}, Shamsi: ${r.ShamsiDate}, Greg: ${r.GregDate}, Debit: ${r.Debit}, Credit: ${r.Credit}`);
    });
  } catch(e) {
