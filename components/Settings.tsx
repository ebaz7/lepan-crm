...
const QRCode = ({ value, size }: { value: string, size: number }) => { 
    ...
    if (error) return (<div className="flex flex-col items-center justify-center text-gray-400 text-xs border-2 border-dashed border-gray-300 rounded-lg p-2" style={{width: size, height: size}}><WifiOff size={24} className="mb-2"/><span className="text-center">امکان نمایش QR وجود ندارد (آفلاین)</span></div>);
...
const Settings: React.FC = () => {
  ...
  const [settings, setSettings] = useState<SystemSettings>({ 
      ...
      exitPermitGroup1Id: '', exitPermitGroup2Id: '' 
  });
  ...
  useEffect(() => { loadSettings(); setNotificationsEnabled(isNotificationEnabledInApp()); checkWhatsappStatus(); loadAppUsers(); }, []);

  const loadSettings = async () => { try { const data = await getSettings(); setSettings({...data, companies: data.companies || [], savedContacts: data.savedContacts || []}); } catch (e) { console.error("Failed to load settings"); } };
  const loadAppUsers = async () => { try { const users = await getUsers(); const contacts = users.filter(u => u.phoneNumber).map(u => ({ id: u.id, name: `(کاربر) ${u.fullName}`, number: u.phoneNumber!, isGroup: false, baleId: u.baleChatId })); setAppUsers(contacts); } catch (e) { console.error("Failed to load users"); } };
  const checkWhatsappStatus = async () => { setRefreshingWA(true); try { const status = await apiCall<{ready: boolean, qr: string | null, user: string | null}>('/whatsapp/status'); setWhatsappStatus(status); } catch (e) { } finally { setRefreshingWA(false); } };
  ...
  const handleWhatsappRestart = async () => { if (!confirm('آیا می‌خواهید سرویس واتساپ را بازنشانی کنید؟')) return; setRestartingWA(true); try { await apiCall('/whatsapp/restart', 'POST'); alert('درخواست بازنشانی ارسال شد.'); setTimeout(checkWhatsappStatus, 3000); } catch (e) { alert('خطا'); } finally { setRestartingWA(false); } };
  ...
  useEffect(() => { let interval: any; if (activeCategory === 'whatsapp' && whatsappStatus && !whatsappStatus.ready) { interval = setInterval(checkWhatsappStatus, 3000); } return () => clearInterval(interval); }, [activeCategory, whatsappStatus]);

  const handleSave = async (e: React.FormEvent) => { e.preventDefault(); setLoading(true); try { let currentCompanies = [...(settings.companies || [])]; if (activeCategory === 'data' && (newCompanyName.trim() || editingCompanyId)) { if (editingCompanyId) { currentCompanies = currentCompanies.map(c => c.id === editingCompanyId ? { ...c, name: newCompanyName.trim(), logo: newCompanyLogo, showInWarehouse: newCompanyShowInWarehouse, banks: newCompanyBanks, letterhead: newCompanyLetterhead, registrationNumber: newCompanyRegNum, nationalId: newCompanyNatId, address: newCompanyAddress, phone: newCompanyPhone, fax: newCompanyFax, postalCode: newCompanyPostalCode, economicCode: newCompanyEcoCode } : c ); } else if (newCompanyName.trim()) { currentCompanies = [...currentCompanies, { id: generateUUID(), name: newCompanyName.trim(), logo: newCompanyLogo, showInWarehouse: newCompanyShowInWarehouse, banks: newCompanyBanks, letterhead: newCompanyLetterhead, registrationNumber: newCompanyRegNum, nationalId: newCompanyNatId, address: newCompanyAddress, phone: newCompanyPhone, fax: newCompanyFax, postalCode: newCompanyPostalCode, economicCode: newCompanyEcoCode }]; } resetCompanyForm(); } const syncedSettings = { ...settings, companies: currentCompanies, companyNames: currentCompanies.map(c => c.name) }; await saveSettings(syncedSettings); setSettings(syncedSettings); setMessage('ذخیره شد ✅'); setTimeout(() => setMessage(''), 3000); } catch (e) { setMessage('خطا ❌'); } finally { setLoading(false); } };
  ...
  const handleToggleNotifications = async () => { const granted = await requestNotificationPermission(); if (granted) { setNotificationPreference(true); setNotificationsEnabled(true); alert("نوتیفیکیشن فعال شد."); } else { alert("دسترسی مسدود است."); } };
  const handleTestNotification = async () => { try { const userStr = localStorage.getItem('app_current_user'); const username = userStr ? JSON.parse(userStr).username : 'test'; await apiCall('/send-test-push', 'POST', { username }); alert("ارسال شد."); } catch (e: any) { alert("خطا: " + e.message); } };
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden flex flex-col md:flex-row min-h-[600px] mb-20 animate-fade-in">
        <div className="w-full md:w-64 bg-gray-50 border-b md:border-b-0 md:border-l border-gray-200 p-4">
            ...
            <nav className="space-y-1">
                ...
                <button onClick={() => setActiveCategory('warehouse')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${activeCategory === 'warehouse' ? 'bg-white shadow text-orange-700 font-bold' : 'text-gray-600 hover:bg-gray-100'}`}><Warehouse size={18}/> انبار و خروج</button>
        ...
        <div className="flex-1 p-6 md:p-8 overflow-y-auto max-h-[calc(100vh-100px)]">
            {activeCategory === 'fiscal' ? (<FiscalYearManager />) : (
                <form onSubmit={handleSave} className="space-y-8 max-w-4xl mx-auto">
                    {/* ... System, Data, Templates, Commerce sections same as before ... */}
                        ...
                        <div className="space-y-8 animate-fade-in">
                             <div className="space-y-4">
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div><input type="file" ref={iconInputRef} className="hidden" accept="image/*" onChange={handleIconChange} /><button type="button" onClick={() => iconInputRef.current?.click()} className="text-blue-600 text-sm hover:underline font-bold" disabled={uploadingIcon}>{uploadingIcon ? '...' : 'تغییر آیکون برنامه'}</button></div>
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <button type="button" onClick={handleToggleNotifications} className={`w-full px-4 py-2 rounded-lg border flex items-center justify-center gap-2 transition-colors ${notificationsEnabled ? 'bg-green-50 border-green-200 text-green-700' : 'bg-gray-50 text-gray-600'}`}>{notificationsEnabled ? <BellRing size={18} /> : <BellOff size={18} />}<span>{notificationsEnabled ? 'نوتیفیکیشن‌ها فعال است' : 'فعال‌سازی نوتیفیکیشن'}</span></button>
                                    <button type="button" onClick={handleTestNotification} className="w-full px-4 py-2 rounded-lg border bg-blue-50 border-blue-200 text-blue-700 flex items-center justify-center gap-2 transition-colors hover:bg-blue-100"><Send size={18}/> <span>ارسال پیام تست</span></button>
                                </div>
                            </div>
                            <div className="space-y-4">
                                ...
                                <div className="bg-amber-50 p-3 rounded-lg border border-amber-200 text-xs text-amber-800 mb-2">نکته: این تنظیمات فقط در صورتی اعمال می‌شود که سال مالی فعال نباشد یا تنظیمی برای شرکت در سال مالی وجود نداشته باشد.</div>
                    ...
                    {activeCategory === 'warehouse' && (
                        <div className="space-y-6 animate-fade-in">
                            <h3 className="font-bold text-gray-800 border-b pb-2 flex items-center gap-2"><Warehouse size={20}/> تنظیمات انبار و خروج</h3>
                            
                            {/* EXIT PERMIT GROUPS CONFIGURATION */}
                            <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 space-y-4">
                                <h4 className="font-bold text-orange-900 text-sm flex items-center gap-2"><Users size={18}/> تنظیمات گروه‌های اطلاع‌رسانی خروج (مهم)</h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        <label className="text-xs font-bold text-gray-700 block mb-1">گروه اول (مدیریتی/فروش):</label>
                                        <p className="text-[10px] text-gray-500 mb-1">دریافت پیام در مراحل: تایید مدیرعامل، خروج نهایی</p>
                                        <select 
                                            className="w-full border rounded-lg p-2 text-sm bg-white" 
                                            value={settings.exitPermitGroup1Id || ''} 
                                            onChange={e => setSettings({...settings, exitPermitGroup1Id: e.target.value})}
                                        >
                                            <option value="">-- انتخاب گروه/شخص --</option>
                                            {settings.savedContacts?.map(c => (
                                                <option key={`g1_${c.number}`} value={c.number}>{c.name} {c.isGroup ? '(گروه)' : ''}</option>
                                            ))}
                                        </select>
                                    </div>
                                    
                                    <div>
                                        <label className="text-xs font-bold text-gray-700 block mb-1">گروه دوم (عملیاتی/انبار):</label>
                                        <p className="text-[10px] text-gray-500 mb-1">دریافت پیام در مراحل: تایید کارخانه، تایید انبار، خروج نهایی</p>
                                        <select 
                                            className="w-full border rounded-lg p-2 text-sm bg-white" 
                                            value={settings.exitPermitGroup2Id || ''} 
                                            onChange={e => setSettings({...settings, exitPermitGroup2Id: e.target.value})}
                                        >
                                            <option value="">-- انتخاب گروه/شخص --</option>
                                            {settings.savedContacts?.map(c => (
                                                <option key={`g2_${c.number}`} value={c.number}>{c.name} {c.isGroup ? '(گروه)' : ''}</option>
                                            ))}
                                        </select>
                                    </div>
                                </div>
                            ...
                            <div className="space-y-4 pt-4 border-t">
                                <div><label className="text-sm font-bold text-gray-700 block mb-1">شماره گروه واتساپ انبار (پیش‌فرض)</label><select className="w-full border rounded-lg p-3 dir-ltr text-left bg-white" value={settings.defaultWarehouseGroup || ''} onChange={e => setSettings({...settings, defaultWarehouseGroup: e.target.value})}><option value="">-- انتخاب گروه --</option>{settings.savedContacts?.filter(c => c.isGroup).map(c => (<option key={c.id} value={c.number}>{c.name} {c.baleId ? '(+Bale)' : ''}</option>))}</select></div>
                                <div><label className="text-sm font-bold text-gray-700 block mb-1">شماره مدیر فروش (پیش‌فرض)</label><input className="w-full border rounded-lg p-3 dir-ltr text-left" value={settings.defaultSalesManager || ''} onChange={e => setSettings({...settings, defaultSalesManager: e.target.value})} placeholder="98912..." /></div>
                            </div>
                        </div>
                    )}
                         ...
                         <div className="space-y-8 animate-fade-in">
                            ...
                            <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                                ...
                                <div className="bg-white border rounded-xl p-3 mb-4">
                                    ...
                                    {tempDualPrint && (<div className="grid grid-cols-1 md:grid-cols-2 gap-2 mb-2 animate-fade-in"><select className="border rounded p-1.5 text-xs bg-red-50" value={tempInternalWithdrawalLayout} onChange={e => setTempInternalWithdrawalLayout(e.target.value)}><option value="">قالب برداشت (Bardasht)</option>{settings.printTemplates?.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select><select className="border rounded p-1.5 text-xs bg-green-50" value={tempInternalDepositLayout} onChange={e => setTempInternalDepositLayout(e.target.value)}><option value="">قالب واریز (Variz)</option>{settings.printTemplates?.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>)}
                                    ...
                                    <div className="space-y-1 mt-2">{newCompanyBanks.map((bank, idx) => (<div key={bank.id || idx} className={`flex justify-between items-center px-2 py-1.5 rounded text-xs border ${editingBankId === bank.id ? 'bg-blue-50 border-blue-300' : 'bg-gray-50'}`}><div className="flex flex-col gap-0.5"><span className="font-bold">{bank.bankName}</span><span className="font-mono text-gray-500">{bank.accountNumber}</span>{bank.formLayoutId && <span className="text-[9px] text-blue-600">قالب چک: {settings.printTemplates?.find(t=>t.id===bank.formLayoutId)?.name}</span>}</div><div className="flex gap-1"><button type="button" onClick={() => editCompanyBank(bank)} className="text-blue-500"><Pencil size={14}/></button><button type="button" onClick={() => removeCompanyBank(bank.id)} className="text-red-400"><X size={14}/></button></div></div>))}</div>
                                </div>
                                ...
                                <div className="space-y-2 mt-6 max-h-64 overflow-y-auto border-t pt-4">{settings.companies?.map(c => (<div key={c.id} className="flex flex-col bg-white p-3 rounded border shadow-sm gap-2"><div className="flex justify-between items-center"><div className="flex items-center gap-2">{c.logo && <img src={c.logo} className="w-6 h-6 object-contain"/>}<span className="text-sm font-bold">{c.name}</span></div><div className="flex gap-1"><button type="button" onClick={() => handleEditCompany(c)} className="text-blue-500 p-1 hover:bg-blue-50 rounded"><Pencil size={14}/></button><button type="button" onClick={() => handleRemoveCompany(c.id)} className="text-red-500 p-1 hover:bg-red-50 rounded"><Trash2 size={14}/></button></div></div></div>))}</div>
                            </div>
                            <div className="bg-white p-4 rounded-xl border border-gray-200"><h3 className="font-bold text-gray-800 border-b pb-2 mb-3">بانک‌های عامل (عمومی)</h3><div className="flex gap-2"><input className="flex-1 border rounded-lg p-2 text-sm" placeholder="نام بانک..." value={newOperatingBank} onChange={(e) => setNewOperatingBank(e.target.value)} /><button type="button" onClick={handleAddOperatingBank} className="bg-indigo-600 text-white px-4 py-2 rounded-lg"><Plus size={20}/></button></div><div className="mt-3 flex flex-wrap gap-2">{settings.operatingBankNames?.map(b => (<div key={b} className="bg-gray-100 px-3 py-1 rounded-full text-xs flex items-center gap-2">{b} <button onClick={() => handleRemoveOperatingBank(b)} className="text-red-500 hover:text-red-700"><X size={12}/></button></div>))}</div></div></div>
                        </div>
                    )}
                    
                    {activeCategory === 'integrations' && (
                        <div className="space-y-6 animate-fade-in"><h3 className="font-bold text-gray-800 border-b pb-2 flex items-center gap-2"><Link size={20}/> تنظیمات اتصالات خارجی</h3><div className="space-y-4"><div><label className="text-sm font-bold text-gray-700 block mb-1">کلید پنل پیامک (API Key)</label><input type="password" className="w-full border rounded-lg p-3 dir-ltr text-left" value={settings.smsApiKey} onChange={e => setSettings({...settings, smsApiKey: e.target.value})} placeholder="API Key..." /></div><div><label className="text-sm font-bold text-gray-700 block mb-1">شماره فرستنده پیامک</label><input type="text" className="w-full border rounded-lg p-3 dir-ltr text-left" value={settings.smsSenderNumber} onChange={e => setSettings({...settings, smsSenderNumber: e.target.value})} placeholder="1000..." /></div><div><label className="text-sm font-bold text-gray-700 block mb-1">Google Calendar ID (اختیاری)</label><input type="text" className="w-full border rounded-lg p-3 dir-ltr text-left" value={settings.googleCalendarId} onChange={e => setSettings({...settings, googleCalendarId: e.target.value})} placeholder="calendar-id..." /></div><div><label className="text-sm font-bold text-gray-700 block mb-1">کلید هوش مصنوعی (Gemini API)</label><input type="password" className="w-full border rounded-lg p-3 dir-ltr text-left" value={settings.geminiApiKey} onChange={e => setSettings({...settings, geminiApiKey: e.target.value})} placeholder="AI Studio Key..." /><p className="text-xs text-gray-500 mt-1">برای استفاده از قابلیت‌های هوشمند (تحلیل متن، استخراج داده از واتساپ و...) نیاز است.</p></div><div><label className="text-sm font-bold text-gray-700 block mb-1">توکن ربات تلگرام</label><input type="password" className="w-full border rounded-lg p-3 dir-ltr text-left" value={settings.telegramBotToken} onChange={e => setSettings({...settings, telegramBotToken: e.target.value})} placeholder="Bot Token..." /></div><div><label className="text-sm font-bold text-gray-700 block mb-1">شناسه عددی مدیر در تلگرام</label><input type="text" className="w-full border rounded-lg p-3 dir-ltr text-left" value={settings.telegramAdminId} onChange={e => setSettings({...settings, telegramAdminId: e.target.value})} placeholder="123456789" /></div></div></div>
                    )}
                    
                    {activeCategory === 'templates' && (
                        <div className="space-y-6 animate-fade-in"><div className="flex justify-between items-center border-b pb-2"><h3 className="font-bold text-gray-800 flex items-center gap-2"><LayoutTemplate size={20}/> مدیریت قالب‌های چاپ (چک)</h3><button type="button" onClick={() => setShowDesigner(true)} className="bg-teal-600 text-white px-4 py-2 rounded-lg flex items-center gap-2 text-sm font-bold hover:bg-teal-700"><Plus size={16}/> طراحی قالب جدید</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{settings.printTemplates?.map(t => (<div key={t.id} className="bg-white p-4 rounded-xl border hover:shadow-md transition-all group relative"><div className="flex justify-between items-start"><div><h4 className="font-bold text-gray-800">{t.name}</h4><p className="text-xs text-gray-500">{t.pageSize} - {t.orientation === 'landscape' ? 'افقی' : 'عمودی'}</p></div><div className="flex gap-2"><button type="button" onClick={() => handleEditTemplate(t)} className="text-blue-500 hover:bg-blue-50 p-2 rounded-lg"><Pencil size={18}/></button><button type="button" onClick={() => handleDeleteTemplate(t.id)} className="text-red-500 hover:bg-red-50 p-2 rounded-lg"><Trash2 size={18}/></button></div></div><div className="mt-4 pt-4 border-t text-xs text-gray-400 flex gap-4"><span>{t.fields.length} فیلد تعریف شده</span>{t.backgroundImage && <span>دارای تصویر پس‌زمینه</span>}</div></div>))}{(!settings.printTemplates || settings.printTemplates.length === 0) && (<div className="col-span-full text-center py-10 text-gray-400 bg-gray-50 rounded-xl border border-dashed border-gray-300">هنوز قالبی طراحی نشده است.</div>)}</div></div>
                    )}
                    
                    {activeCategory === 'commerce' && (
                        <div className="space-y-6 animate-fade-in"><h3 className="font-bold text-gray-800 border-b pb-2 flex items-center gap-2"><Container size={20}/> تنظیمات بازرگانی</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-6"><div className="bg-white p-4 rounded-xl border"><h4 className="font-bold text-sm mb-3">گروه‌های کالایی</h4><div className="flex gap-2 mb-2"><input className="flex-1 border rounded-lg p-2 text-sm" placeholder="نام گروه..." value={newCommodity} onChange={e => setNewCommodity(e.target.value)} /><button type="button" onClick={handleAddCommodity} className="bg-blue-600 text-white p-2 rounded-lg"><Plus size={20}/></button></div><div className="flex flex-wrap gap-2">{settings.commodityGroups.map(g => (<div key={g} className="bg-blue-50 text-blue-700 px-3 py-1 rounded-full text-xs flex items-center gap-2 border border-blue-100">{g} <button onClick={() => handleRemoveCommodity(g)} className="hover:text-red-500"><X size={12}/></button></div>))}</div></div><div className="bg-white p-4 rounded-xl border"><h4 className="font-bold text-sm mb-3">شرکت‌های بیمه (طرف قرارداد)</h4><div className="flex gap-2 mb-2"><input className="flex-1 border rounded-lg p-2 text-sm" placeholder="نام شرکت بیمه..." value={newInsuranceCompany} onChange={e => setNewInsuranceCompany(e.target.value)} /><button type="button" onClick={handleAddInsuranceCompany} className="bg-indigo-600 text-white p-2 rounded-lg"><Plus size={20}/></button></div><div className="flex flex-wrap gap-2">{settings.insuranceCompanies?.map(c => (<div key={c} className="bg-indigo-50 text-indigo-700 px-3 py-1 rounded-full text-xs flex items-center gap-2 border border-indigo-100">{c} <button onClick={() => handleRemoveInsuranceCompany(c)} className="hover:text-red-500"><X size={12}/></button></div>))}</div></div></div></div>
                    )}
                    
                    {activeCategory === 'permissions' && (
                        <div className="space-y-8 animate-fade-in"><h3 className="font-bold text-gray-800 border-b pb-2 flex items-center gap-2"><ShieldCheck size={20}/> مدیریت دسترسی نقش‌ها</h3><RolePermissionsEditor settings={settings} onUpdateSettings={handleUpdateSettings} /></div>
                    )}

                    {activeCategory === 'whatsapp' && (
                        <div className="space-y-6 animate-fade-in"><div className="flex justify-between items-center border-b pb-2"><h3 className="font-bold text-gray-800 flex items-center gap-2"><MessageCircle size={20}/> مدیریت پیام‌رسان‌ها (واتساپ و بله)</h3><div className="flex gap-2"><button type="button" onClick={handleFetchGroups} className="text-xs bg-indigo-50 text-indigo-700 px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-indigo-100">{fetchingGroups ? <Loader2 size={14} className="animate-spin"/> : <RefreshCw size={14}/>} بروزرسانی گروه‌ها</button></div></div><div className={`bg-${whatsappStatus?.ready ? 'green' : 'amber'}-50 border border-${whatsappStatus?.ready ? 'green' : 'amber'}-200 rounded-xl p-6 flex flex-col md:flex-row items-center gap-6`}>{refreshingWA ? (<div className="flex flex-col items-center gap-2 text-gray-500"><Loader2 size={32} className="animate-spin"/><span className="text-sm">در حال بررسی وضعیت...</span></div>) : whatsappStatus?.ready ? (<><div className="bg-green-100 p-4 rounded-full text-green-600"><Check size={32}/></div><div className="flex-1 text-center md:text-right"><h3 className="font-bold text-lg text-green-800 mb-1">واتساپ متصل است</h3><p className="text-sm text-green-700">شماره متصل: {whatsappStatus.user ? `+${whatsappStatus.user}` : 'ناشناس'}</p></div><button type="button" onClick={handleWhatsappLogout} className="bg-red-50 text-red-600 border border-red-200 px-4 py-2 rounded-lg text-sm font-bold hover:bg-red-100 transition-colors">خروج از حساب</button></>) : (<><div className="bg-white p-2 rounded-lg border shadow-sm">{whatsappStatus?.qr ? <QRCode value={whatsappStatus.qr} size={160} /> : <div className="w-40 h-40 flex items-center justify-center text-gray-400 text-xs">در حال دریافت QR...</div>}</div><div className="flex-1"><h3 className="font-bold text-lg text-amber-800 mb-2">اتصال به واتساپ</h3><ol className="list-decimal list-inside text-sm text-gray-600 space-y-1"><li>واتساپ را در گوشی خود باز کنید</li><li>به تنظیمات و سپس Linked Devices بروید</li><li>دکمه Link a Device را بزنید</li><li>کد QR روبرو را اسکن کنید</li></ol><div className="flex gap-2 mt-4"><button type="button" onClick={checkWhatsappStatus} className="text-blue-600 text-xs font-bold hover:underline">بروزرسانی وضعیت</button><button type="button" onClick={handleWhatsappRestart} disabled={restartingWA} className="flex items-center gap-1 text-xs bg-red-100 text-red-700 px-3 py-1.5 rounded-lg hover:bg-red-200 font-bold transition-colors">{restartingWA ? <Loader2 size={12} className="animate-spin"/> : <RefreshCcw size={12}/>} بازنشانی و دریافت QR جدید</button></div></div></>)}</div><div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mt-4"><h3 className="font-bold text-gray-800 flex items-center gap-2 mb-3"><Send size={20} className="text-blue-500"/> اتصال به پیام‌رسان بله (Bale)</h3><div className="space-y-2"><label className="text-sm font-bold text-gray-700">توکن ربات بله (Bot Token)</label><input type="password" className="w-full border rounded-lg p-2 dir-ltr text-left font-mono text-sm" placeholder="123456:ABC-DEF..." value={settings.baleBotToken || ''} onChange={(e) => setSettings({...settings, baleBotToken: e.target.value})} /><div className="text-xs text-gray-500 leading-relaxed bg-blue-50 p-3 rounded-lg border border-blue-100"><div className="flex items-center gap-1 font-bold text-blue-800 mb-1"><Info size={14}/> راهنما:</div>برای ارسال پیام به یک گروه در بله (هنگام تایید اسناد):<ol className="list-decimal list-inside mt-1 space-y-1"><li>یک ربات در بله بسازید (@BotFather) و توکن را در بالا وارد کنید.</li><li>ربات را در گروه بله خود عضو کنید و دسترسی ادمین بدهید.</li><li>شناسه عددی گروه بله را پیدا کنید.</li><li>در پایین (دفترچه تلفن)، گروه واتساپی مربوطه را <strong>ویرایش</strong> کنید و <strong>شناسه بله</strong> را وارد نمایید.</li></ol></div></div></div><div className="space-y-4"><h3 className="font-bold text-gray-800 border-b pb-2">دفترچه تلفن هوشمند (گروه‌ها و اشخاص)</h3><div className="flex gap-2 items-end bg-gray-50 p-3 rounded-lg border border-gray-200 flex-wrap"><div className="flex-1 min-w-[150px] space-y-1"><label className="text-xs text-gray-500">نام مخاطب / گروه</label><input className="w-full border rounded-lg p-2 text-sm" placeholder="نام..." value={contactName} onChange={(e) => setContactName(e.target.value)} /></div><div className="flex-1 min-w-[150px] space-y-1"><label className="text-xs text-gray-500">شماره / شناسه گروه (واتساپ)</label><input className="w-full border rounded-lg p-2 text-sm dir-ltr text-left" placeholder="98912..." value={contactNumber} onChange={e => setContactNumber(e.target.value)} /></div><div className="flex-1 min-w-[120px] space-y-1"><label className="text-xs text-gray-500 text-blue-600 font-bold">شناسه بله (Bale ID)</label><input className="w-full border rounded-lg p-2 text-sm dir-ltr text-left border-blue-200" placeholder="12345678" value={contactBaleId} onChange={(e) => setContactBaleId(e.target.value)} /></div><div className="flex items-center gap-2 mb-2"><input type="checkbox" checked={isGroupContact} onChange={e => setIsGroupContact(e.target.checked)} className="w-4 h-4 text-blue-600"/><span className="text-sm">گروه است؟</span></div><div className="flex gap-1 h-[38px]">{editingContactId && <button type="button" onClick={resetContactForm} className="bg-gray-200 text-gray-700 p-2 rounded-lg hover:bg-gray-300" title="انصراف"><X size={20}/></button>}<button type="button" onClick={handleAddOrUpdateContact} className={`${editingContactId ? 'bg-amber-600 hover:bg-amber-700' : 'bg-blue-600 hover:bg-blue-700'} text-white p-2 rounded-lg`}>{editingContactId ? <Save size={20}/> : <Plus size={20}/>}</button></div></div><div className="space-y-2 max-h-60 overflow-y-auto">{settings.savedContacts?.map(c => (<div key={c.id} className={`flex justify-between items-center p-3 bg-white border rounded-lg hover:bg-gray-50 ${editingContactId === c.id ? 'border-amber-400 bg-amber-50' : ''}`}><div className="flex items-center gap-3"><div className={`p-2 rounded-full ${c.isGroup ? 'bg-orange-100 text-orange-600' : 'bg-blue-100 text-blue-600'}`}>{c.isGroup ? <Users size={16} /> : <Smartphone size={16} />}</div><div><div className="font-bold text-sm text-gray-800">{c.name}</div><div className="text-xs text-gray-500 font-mono">WA: {c.number} {c.baleId ? <span className="text-blue-600 font-bold bg-blue-50 px-1 rounded">| Bale: {c.baleId}</span> : ''}</div></div></div><div className="flex gap-2"><button type="button" onClick={() => handleEditContact(c)} className="text-amber-500 hover:text-amber-700 bg-amber-50 p-1.5 rounded"><Pencil size={14} /></button><button type="button" onClick={() => handleDeleteContact(c.id)} className="text-red-400 hover:text-red-600 bg-red-50 p-1.5 rounded"><Trash2 size={14} /></button></div></div>))}{(!settings.savedContacts || settings.savedContacts.length === 0) && <div className="text-center text-gray-400 py-4 text-sm">مخاطبی ثبت نشده است.</div>}</div></div></div>
                    )}
