...
const ManageExitPermits: React.FC<{ currentUser: User, statusFilter?: any }> = ({ currentUser, statusFilter }) => {
    ...
    const handleDelete = async (p: ExitPermit) => {
        ...
        setProcessingId(p.id);
        setCapturePermit({ ...p, status: 'REJECTED' as any }); 
        setTimeout(async () => {
                ...
                <div className="fixed -left-[3000px] top-0">
                    <div id={`capture-permit-${capturePermit.id}`}>
                        <PrintExitPermit 
                            ...
                            onClose={()=>{}} 
                            embed 
                            watermark={processingId && capturePermit.status === 'REJECTED' as any ? 'DELETED' : (capturePermit.updatedAt ? 'EDITED' : null)} 
                        />
                    </div>
                </div>
            )}
            ...
            {editPermit && (
                <EditExitPermitModal 
                    ...
                    onSave={async (updated) => { 
                        // The Edit modal now handles logic but we need to trigger the correction broadcast
