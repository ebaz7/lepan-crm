...
const EditOrderModal: React.FC<EditOrderModalProps> = ({ order, onClose, onSave }) => {
  ...
  const updateBanksForCompany = (companyName: string, currentSettings: SystemSettings) => {
      ...
      // 2. Fallback operating banks ONLY if no company-specific bank exists
      if (bankSet.size === 0) {
      ...
      // 3. Default Iranian banks fallback
      if (bankSet.size === 0) {
  ...
  return (
    <div ref={containerRef} className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-start justify-center p-4 sm:pt-10 overflow-y-auto w-full">
        ...
        <div className="glass-panel rounded-2xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2"><label className="text-sm font-medium text-gray-700">شرکت پرداخت کننده</label><select className="w-full border border-gray-300 rounded-xl px-4 py-3 glass-panel" value={payingCompany} onChange={handleCompanyChange}><option value="">-- انتخاب کنید --</option>{availableCompanies.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200/50 dark:border-white/10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end mb-4 glass-panel p-4 rounded-xl border border-gray-200 shadow-sm">
                        ...
                        {(newLine.method === PaymentMethod.CHEQUE || newLine.method === PaymentMethod.TRANSFER || newLine.method === PaymentMethod.SHEBA || newLine.method === PaymentMethod.SATNA || newLine.method === PaymentMethod.INTERNAL_TRANSFER) ? (
                            <>
                                ...
                                <div className="md:col-span-2 space-y-1"><label className="text-xs text-gray-500">نام بانک مبدا</label><div className="flex gap-1"><select className="w-full border rounded-lg p-2 text-sm glass-panel" value={newLine.bankName} onChange={e => setNewLine({ ...newLine, bankName: e.target.value })}><option value="">-- انتخاب --</option>{availableBanks.map(b => <option key={b} value={b}>{b}</option>)}</select><button type="button" onClick={openAddBankModal} className="bg-blue-100 text-blue-600 rounded-lg px-2 hover:bg-blue-200 border border-blue-200" title="افزودن بانک جدید"><Plus size={16}/></button></div></div>
                            </>
                ...
                <div className="bg-gray-50 p-6 rounded-2xl border border-gray-200">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <label htmlFor="attachment-edit" className={`glass-panel border-2 border-dashed border-gray-300 text-gray-600 px-6 py-3 rounded-xl cursor-pointer hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all flex items-center gap-2 text-sm font-medium ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}>
