      ...
      >
        ...
        <div className={`mt-auto ${isCompact ? 'pt-0.5' : 'pt-2'} border-t-2 border-gray-800 relative z-10`}>
            <div className="grid grid-cols-4 gap-2 text-center">
                ...
                <div className={`flex flex-col items-center justify-end ${isCompact ? 'min-h-[35px]' : 'min-h-[60px]'}`}><div className="mb-0.5 flex items-center justify-center h-full">{(order.approverFinancial || [OrderStatus.APPROVED_FINANCE, OrderStatus.APPROVED_MANAGER, OrderStatus.APPROVED_CEO, OrderStatus.READY_TO_PAY, OrderStatus.PAID].includes(order.status)) ? <Stamp name={order.approverFinancial || 'تایید شده'} title="تایید مالی" /> : <span className="text-gray-300 text-[8px]">امضا نشده</span>}</div><div className="w-full border-t border-gray-400 pt-0.5"><span className="text-[8px] font-bold text-gray-600">مدیر مالی</span></div></div>
                <div className={`flex flex-col items-center justify-end ${isCompact ? 'min-h-[35px]' : 'min-h-[60px]'}`}><div className="mb-0.5 flex items-center justify-center h-full">{(order.approverManager || [OrderStatus.APPROVED_MANAGER, OrderStatus.APPROVED_CEO, OrderStatus.READY_TO_PAY, OrderStatus.PAID].includes(order.status)) ? <Stamp name={order.approverManager || 'تایید شده'} title="تایید مدیریت" /> : <span className="text-gray-300 text-[8px]">امضا نشده</span>}</div><div className="w-full border-t border-gray-400 pt-0.5"><span className="text-[8px] font-bold text-gray-600">مدیریت</span></div></div>
                <div className={`flex flex-col items-center justify-end ${isCompact ? 'min-h-[35px]' : 'min-h-[60px]'}`}><div className="mb-0.5 flex items-center justify-center h-full">{(order.approverCeo || [OrderStatus.APPROVED_CEO, OrderStatus.READY_TO_PAY, OrderStatus.PAID].includes(order.status)) ? <Stamp name={order.approverCeo || 'تایید شده'} title="مدیر عامل" /> : <span className="text-gray-300 text-[8px]">امضا نشده</span>}</div><div className="w-full border-t border-gray-400 pt-0.5"><span className="text-[8px] font-bold text-gray-600">مدیر عامل</span></div></div>
            </div>
        </div>
      </div>
  );
