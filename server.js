...
const sendDailySalesReportForDate = async (db, dateObj, labelSuffix = '', targetsOverride = null) => {
    ...
    cron.schedule('30 15 * * *', async () => {
        ...
        // 2. AUTOMATED DAILY PRODUCTION REPORT
        const prodTargets = [];
        if (settings.productionTelegramGroupId) prodTargets.push({ platform: 'telegram', id: settings.productionTelegramGroupId });
        if (settings.productionBaleGroupId) prodTargets.push({ platform: 'bale', id: settings.productionBaleGroupId });

        if (prodTargets.length > 0) {
            console.log(`[Cron 19:00] Preparing Automated Production Report for ${prodTargets.length} targets...`);
            try {
                ...
                const sql = `
                    ...
                    WHERE RTRIM(LTRIM(t10.Field_009)) IN ('61', '67', '79', '73')
                      AND t10.Field_008 >= '${gregDate}T00:00:00.000Z'
                      AND t10.Field_008 <= '${gregDate}T23:59:59.999Z'
                ...
                const totals = { qty_61, qty_67, qty_79, qty_73, grandTotal: qty_61 + qty_67 + qty_79 + qty_73 };

                const key = `${shamsiDate}_${shamsiDate}`;
                db.productionReportWastes = db.productionReportWastes || {};
                const waste = db.productionReportWastes[key] || {
                    waste_61: 0, waste_67: 0, waste_79: 0, waste_73: 0,
                    pct_61: 0, pct_67: 0, pct_79: 0, pct_73: 0,
                    totalWaste: 0, totalPct: 0, details: ''
                };

                const prodTitle = `گزارش آمار کل تولید و ضایعات (${shamsiDate})`;
...
// --- SYNC COMPANY BASE DATA FROM LIVE SAYAN DB ---
app.post('/api/sayan/sync-companies', async (req, res) => {
    try {
        const db = getDb();
        const settings = db.settings || {};
        ...
        if (settings.sayanApiUrl && settings.sayanApiKey) {
            try {
                const gnrSql = `
                    SELECT 
                        t1.Field_001 as Id,
                        t1.Field_004 as Code,
                        RTRIM(LTRIM(COALESCE(t1.Field_006, '') + ' ' + COALESCE(t1.Field_005, ''))) as FullName,
                        RTRIM(LTRIM(t1.Field_006)) as CompanyName,
                        RTRIM(LTRIM(t1.Field_005)) as Prefix,
                        t1.Field_007 as NationalId,
                        t1.Field_008 as RegistrationNumber,
                        t1.Field_013 as Phone,
                        t1.Field_014 as Mobile,
                        t1.Field_015 as Address,
                        t1.Field_016 as PostalCode,
                        t1.Field_028 as Fax,
                        t1.Field_030 as AccountNumber,
                        t1.Field_031 as CardNumber,
                        t1.Field_032 as Sheba
                    FROM GNR_TBL_001 t1
                    WHERE (t1.Field_006 IS NOT NULL AND RTRIM(LTRIM(t1.Field_006)) <> '')
                       OR (t1.Field_005 IS NOT NULL AND RTRIM(LTRIM(t1.Field_005)) <> '')
                `;
                const gnrRows = await executeSayanQuery(db, gnrSql).catch((e) => {
                    console.warn('[Sync Companies] GNR Query error:', e.message);
                    return [];
                });

                const actSql = `
                    SELECT 
                        t07.Field_005 as Code,
                        RTRIM(LTRIM(t07.Field_006)) as AccountName,
                        t07.Field_007 as Phone,
                        t07.Field_008 as Address,
                        t07.Field_009 as NationalId
                    FROM ACT_TBL_007 t07
                    WHERE t07.Field_006 IS NOT NULL AND RTRIM(LTRIM(t07.Field_006)) <> ''
                `;
                const actRows = await executeSayanQuery(db, actSql).catch(() => []);

                const tbl008Sql = `
                    SELECT 
                        Field_003 as BankName,
                        Field_005 as RawText
                    FROM TBL_008
                `;
                const tbl008Rows = await executeSayanQuery(db, tbl008Sql).catch(() => []);

                const companyMap = new Map();

                function getOrAddComp(name) {
                    ...
                    if (!companyMap.has(clean)) {
                        companyMap.set(clean, {
                            id: 'comp_sys_' + Math.random().toString(36).substring(2, 9),
                            name: clean,
                    ...
                    return companyMap.get(clean);
                }

                if (Array.isArray(gnrRows)) {
                    gnrRows.forEach(r => {
                        const name = (r.FullName || r.CompanyName || r.Prefix || '').trim();
                        const comp = getOrAddComp(name);
                        if (comp) {
                            if (r.NationalId && String(r.NationalId).trim() !== '0' && !comp.nationalId) comp.nationalId = String(r.NationalId).trim();
                            if (r.RegistrationNumber && !comp.registrationNumber) comp.registrationNumber = String(r.RegistrationNumber).trim();
                            if ((r.Phone || r.Mobile) && !comp.phone) comp.phone = String(r.Phone || r.Mobile).trim();
                            if (r.Fax && !comp.fax) comp.fax = String(r.Fax).trim();
                            if (r.PostalCode && !comp.postalCode) comp.postalCode = String(r.PostalCode).trim();
                            if (r.Address && !comp.address) comp.address = String(r.Address).trim();

                            const accNo = String(r.AccountNumber || '').trim();
                            const cardNo = String(r.CardNumber || '').trim();
                            const sheba = String(r.Sheba || '').trim();
                ...
                if (Array.isArray(actRows)) {
                    actRows.forEach(r => {
                        ...
                        if (name && !name.startsWith('11') && !name.startsWith('31')) {
                            const comp = getOrAddComp(name);
                            if (comp) {
                                if (r.NationalId && !comp.nationalId) comp.nationalId = String(r.NationalId).trim();
                                if (r.Phone && !comp.phone) comp.phone = String(r.Phone).trim();
                                if (r.Address && !comp.address) comp.address = String(r.Address).trim();
                            }
                        }
                    });
                ...
                if (Array.isArray(tbl008Rows)) {
                    tbl008Rows.forEach(r => {
                        ...
                        if (raw.includes('&')) {
                            ...
                            if (parts.length >= 5) {
                                ...
                                if (compName) {
                                    const comp = getOrAddComp(compName);
                                    if (comp) {
                ...
                syncedCompanies = Array.from(companyMap.values());
            } catch (liveErr) {
                console.warn('[Sync Companies] Live Sayan query failed, falling back to local storage:', liveErr.message);
                source = 'fallback_local';
            }
        }

        if (syncedCompanies.length === 0) {
            syncedCompanies = dbManager.extractExternalCompanyDetails(db);
        }

        const existingCompanies = Array.isArray(db.settings.companies) ? db.settings.companies : [];
        const finalMap = new Map();

        existingCompanies.forEach(c => {
            if (c && c.name && c.name.trim()) {
                const cleanName = c.name.trim();
                if (cleanName !== 'd _' && cleanName !== 'e' && cleanName.length >= 2) {
                    finalMap.set(cleanName, { ...c });
                }
            }
        });

        syncedCompanies.forEach(sc => {
            if (!sc || !sc.name) return;
            const cleanName = sc.name.trim();
            if (cleanName === 'd _' || cleanName === 'e' || cleanName.length < 2) return;

            if (finalMap.has(cleanName)) {
                const existing = finalMap.get(cleanName);
                if (!existing.registrationNumber && sc.registrationNumber) existing.registrationNumber = sc.registrationNumber;
                if (!existing.nationalId && sc.nationalId) existing.nationalId = sc.nationalId;
                if (!existing.economicCode && sc.economicCode) existing.economicCode = sc.economicCode;
                if (!existing.address && sc.address) existing.address = sc.address;
                if (!existing.phone && sc.phone) existing.phone = sc.phone;
                if (!existing.fax && sc.fax) existing.fax = sc.fax;
                if (!existing.postalCode && sc.postalCode) existing.postalCode = sc.postalCode;

                if (Array.isArray(sc.banks) && sc.banks.length > 0) {
                    if (!Array.isArray(existing.banks)) existing.banks = [];
                    sc.banks.forEach(sb => {
                        const hasBank = existing.banks.some(b => 
                            (sb.accountNumber && b.accountNumber === sb.accountNumber) ||
                            (sb.sheba && b.sheba === sb.sheba)
                        );
                        if (!hasBank) {
                            existing.banks.push(sb);
                        }
                    });
                }
            } else {
                finalMap.set(cleanName, sc);
            }
        });

        const mergedCompaniesList = Array.from(finalMap.values());
        const mergedCompanyNames = mergedCompaniesList.map(c => c.name);

        db.settings.companies = mergedCompaniesList;
        db.settings.companyNames = mergedCompanyNames;
        ...
        res.json({
            success: true,
            source,
            count: mergedCompaniesList.length,
            message: `اطلاعات پایه ${mergedCompaniesList.length} شرکت از دیتابیس با موفقیت استخراج و بارگذاری شد.`,
            companies: mergedCompaniesList
        });
    } catch (e) {
        console.error('[Sync Companies Error]', e);
        res.status(500).json({ error: e.message || 'خطا در بارگذاری اطلاعات پایه شرکت‌ها' });
    }
});
