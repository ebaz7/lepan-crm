...
// 6. Sales & Return Report PDF
export const generateSalesReportPDF = async (
  title,
  label,
  items = [],
  summaryStats = { salesQty: 0, salesAmt: 0, returnQty: 0, returnAmt: 0, netQty: 0, netAmt: 0, avgPricePerKg: 0 }
) => {
  try {
    const browser = await getBrowser();
    const page = await browser.newPage();
    ...
    const rowsHtml = items.map((item, idx) => `
      <tr style="background-color: ${idx % 2 === 0 ? "#ffffff" : "#f8fafc"}; font-size: 9pt;">
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold; color: #64748b;">${idx + 1}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: right; font-weight: bold; color: #1e293b;">${item.groupName || item.itemName || "کالا"}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: center; color: #1e40af;">${fmtNum(item.salesQty, 1)}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: left; color: #1e40af;">${fmtNum(item.salesAmt)}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: center; color: #991b1b;">${item.returnQty > 0 ? fmtNum(item.returnQty, 1) : "-"}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: left; color: #991b1b;">${item.returnAmt > 0 ? fmtNum(item.returnAmt) : "-"}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold; background: #ecfdf5; color: #065f46;">${fmtNum(item.netQty, 1)}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: left; font-weight: bold; background: #ecfdf5; color: #065f46;">${fmtNum(item.netAmt)}</td>
        <td style="padding: 6px 8px; border: 1px solid #cbd5e1; text-align: center; font-weight: bold; background: #f0fdf4; color: #15803d;">${fmtNum(item.avgPricePerKg)}</td>
      </tr>
    `).join("");
    ...
    <head>
        ...
        <style>
            ...
            .stat-card { flex: 1; border: 1px solid #cbd5e1; padding: 8px 12px; border-radius: 6px; background: #f8fafc; text-align: center; }
            .stat-card.sales { border-color: #93c5fd; background: #eff6ff; }
            .stat-card.return { border-color: #fca5a5; background: #fef2f2; }
            .stat-card.net { border-color: #6ee7b7; background: #ecfdf5; }
            .stat-card.avg { border-color: #fde047; background: #fefce8; }
            .stat-title { font-size: 8.5pt; font-weight: bold; color: #475569; }
            .stat-value { font-size: 12pt; font-weight: 800; margin-top: 4px; color: #0f172a; }
            ...
            .report-table th, .report-table td { border: 1px solid #64748b; padding: 6px 8px; }
            .report-table th { background-color: #1e293b; color: #ffffff; font-weight: 800; text-align: center; }
            .summary-row { background-color: #f1f5f9; font-weight: bold; }
        </style>
    </head>
    <body>
        <table class="header-table">
            <tr>
                <td style="width: 25%;">
                    <div style="border: 1px solid #94a3b8; padding: 6px 10px; border-radius: 4px; font-size: 9pt; background: #f8fafc;">
                        <strong>تاریخ / بازه:</strong> ${label}
                    </div>
                </td>
                <td style="width: 50%;" class="title">
                    ${title || "گزارش تحلیلی فروش و مرجوعی سایان ERP"}
                </td>
        ...
        <div class="stats-grid">
            <div class="stat-card sales">
                <div class="stat-title" style="color: #1e40af;">فروش ناخالص</div>
                <div class="stat-value" style="color: #1e3a8a;">${fmtNum(summaryStats.salesAmt)} <span style="font-size: 8pt;">ریال</span></div>
                <div class="stat-sub">وزن: ${fmtNum(summaryStats.salesQty, 1)} کیلوگرم</div>
            </div>
            <div class="stat-card return">
                <div class="stat-title" style="color: #991b1b;">مرجوعی (برگشت از فروش)</div>
                <div class="stat-value" style="color: #7f1d1d;">${fmtNum(summaryStats.returnAmt)} <span style="font-size: 8pt;">ریال</span></div>
                <div class="stat-sub">وزن: ${fmtNum(summaryStats.returnQty, 1)} کیلوگرم</div>
            </div>
            <div class="stat-card net">
                <div class="stat-title" style="color: #065f46;">فروش خالص</div>
                <div class="stat-value" style="color: #047857;">${fmtNum(summaryStats.netAmt)} <span style="font-size: 8pt;">ریال</span></div>
                <div class="stat-sub">وزن خالص: ${fmtNum(summaryStats.netQty, 1)} کیلوگرم</div>
            </div>
            <div class="stat-card avg">
                <div class="stat-title" style="color: #854d0e;">جدول میانگین قیمت</div>
                <div class="stat-value" style="color: #a16207;">${fmtNum(summaryStats.avgPricePerKg)} <span style="font-size: 8pt;">ریال / کیلوگرم</span></div>
                <div class="stat-sub">مبلغ خالص ÷ وزن خالص</div>
            </div>
        </div>
        ...
        <table class="report-table">
            <thead>
                <tr>
                    <th style="width: 4%;">#</th>
                    <th style="width: 20%;">گروه کالا / محصول</th>
                    <th style="width: 10%;">وزن فروش (ک‌گ)</th>
                    <th style="width: 13%;">مبلغ فروش (ریال)</th>
                    <th style="width: 10%;">وزن مرجوعی (ک‌گ)</th>
                    <th style="width: 13%;">مبلغ مرجوعی (ریال)</th>
                    <th style="width: 10%;">وزن خالص (ک‌گ)</th>
                    <th style="width: 13%;">مبلغ خالص (ریال)</th>
                    <th style="width: 12%;">میانگین قیمت (ریال/ک‌گ)</th>
                </tr>
            </thead>
            <tbody>
                ${rowsHtml || "<tr><td colspan='9' style='text-align: center;'>هیچ داده‌ای ثبت نشده است.</td></tr>"}
            </tbody>
            <tfoot>
                <tr class="summary-row" style="background: #e2e8f0; font-size: 10pt;">
                    <td colspan="2" style="text-align: right; font-weight: bold; padding-right: 12px;">جمع کل (برآیند)</td>
                    <td style="text-align: center; font-weight: bold; color: #1e40af;">${fmtNum(summaryStats.salesQty, 1)}</td>
                    <td style="text-align: left; font-weight: bold; color: #1e40af;">${fmtNum(summaryStats.salesAmt)}</td>
                    <td style="text-align: center; font-weight: bold; color: #991b1b;">${fmtNum(summaryStats.returnQty, 1)}</td>
                    <td style="text-align: left; font-weight: bold; color: #991b1b;">${fmtNum(summaryStats.returnAmt)}</td>
                    <td style="text-align: center; font-weight: bold; background: #a7f3d0; color: #065f46;">${fmtNum(summaryStats.netQty, 1)}</td>
                    <td style="text-align: left; font-weight: bold; background: #a7f3d0; color: #065f46;">${fmtNum(summaryStats.netAmt)}</td>
                    <td style="text-align: center; font-weight: bold; background: #fef08a; color: #854d0e;">${fmtNum(summaryStats.avgPricePerKg)}</td>
                </tr>
            </tfoot>
        </table>
    </body>
    ...
    console.error("Generate Sales Report PDF Error:", e);
    throw e;
  }
};
