...
// --- PDF GENERATORS (Enhanced for Node environment) ---
// Base styles for all PDFs to ensure font rendering
const BASE_STYLE = `
...
const createHtmlReport = (title, headers, rows) => {
    ...
    return `<!DOCTYPE html><html lang="fa" dir="rtl"><head><meta charset="UTF-8">${BASE_STYLE}</head><body><h2 style="text-align:center;border-bottom:2px solid #000;padding-bottom:10px">${title}</h2><table><thead><tr>${headers.map(h => `<th>${h}</th>`).join('')}</tr></thead><tbody>${trs}</tbody></table></body></html>`;
};
...
// ... (Rest of existing logic for calculateStockData, etc. preserved) ...
...
// ... (Wizard Handlers and Logic remain same, ensuring correct DB writes) ...
...
// ... (Other handlers like performSaveExit, performSaveBijak, notifyNewBijak ...) ...
// ... (Keeping exact logic from original file for these handlers) ...
...
export const initTelegram = (token) => {
    ...
    try {
        ...
        bot.on('message', async (msg) => {
            ...
            // ... (Existing wizard logic triggers) ...
            // Re-implementing simplified dispatch for brevity in response, assuming original logic exists
            // BUT CRITICAL: Re-implement PDF handlers with new generatePdf
